...
import { incomingQ, deadQ } from '@/lib/queue'
...
// Tipos para el payload del job
interface MessageJobData {
  botId: string
  channelId: string
  channelType: 'whatsapp' | 'telegram' | 'webchat'
...
async function getChannelConfig(channelId: string) {
...
async function getProviderConfig(providerId: string) {
  const provider = await prisma.prisma.findUnique({
    where: { id: providerId }
  })
...
async function sendWebchatResponse(
  channelId: string,
  externalUserId: string,
  text: string
) {
  // Por ahora solo guardamos en la base de datos
  // En el futuro se puede implementar SSE o WebSockets
  await saveMessage({
...
// Iniciar el worker si se ejecuta directamente
