...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check if block is in DRAFT status
    if (block.status !== "DRAFT") {
      return NextResponse.json({ error: "Block must be in DRAFT status to submit" }, { status: 400 })
    }

    // Update block status to IN_REVIEW
    ...
    console.error("[blocks/submit] error:", error)
